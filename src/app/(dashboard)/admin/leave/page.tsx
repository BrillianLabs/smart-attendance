      ...
      <div className="bg-surface-container-lowest rounded-[2.5rem] shadow-sm shadow-primary/5 border border-outline-variant/10 overflow-hidden">
        {leaves.length === 0 ? (
          ...
          <div className="divide-y divide-surface-container-low">
            {leaves.map(leave => (
              <div key={leave.id} className="p-5 sm:p-8 lg:p-10 hover:bg-surface-container-low/30 transition-all group">
                <div className="flex flex-col gap-6">
                  {/* Info Utama */}
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-surface-container-low overflow-hidden flex-shrink-0 flex items-center justify-center font-black text-primary border border-outline-variant/5">
                    ...
                    <div className="space-y-1 min-w-0 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-base sm:text-lg font-bold text-on-surface leading-tight">{leave.profiles?.full_name ?? 'Anggota Tidak Dikenal'}</p>
                        <Badge variant="gray" className="px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest opacity-60 shrink-0">
                          {leave.leave_type}
                        </Badge>
                      </div>
                  ...
                  {/* Alasan & Dokumen */}
                  <div className="p-4 sm:p-5 bg-surface rounded-2xl border border-outline-variant/10 italic text-sm text-on-surface-variant leading-relaxed">
                    "{leave.reason}"
                  ...
                  {/* Tombol Aksi — full width di mobile, sejajar di desktop */}
                  <div className="flex items-center justify-between gap-3 pt-2 border-t border-surface-container-low">
                    <Badge variant={statusVariant(leave.status)} className="px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm shrink-0">
